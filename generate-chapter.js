// src/pages/api/projects/[id]/generate-chapter.js
...
async function handler(req, res) {
  ...
  if (chapterNumber > project.maxChapters) return res.status(400).json({ error: 'Chapter exceeds package limit' })

  try {
    const text = await generateChapter(project.outline, chapterNumber, prevSummary)
    const wordCount = text.trim().split(/\s+/).filter(Boolean).length

    // Get title from outline or fallback
